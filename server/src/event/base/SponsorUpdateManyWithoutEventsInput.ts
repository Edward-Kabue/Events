/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/docs/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { SponsorWhereUniqueInput } from "../../sponsor/base/SponsorWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";
@InputType()
class SponsorUpdateManyWithoutEventsInput {
  @Field(() => [SponsorWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [SponsorWhereUniqueInput],
  })
  connect?: Array<SponsorWhereUniqueInput>;

  @Field(() => [SponsorWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [SponsorWhereUniqueInput],
  })
  disconnect?: Array<SponsorWhereUniqueInput>;

  @Field(() => [SponsorWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [SponsorWhereUniqueInput],
  })
  set?: Array<SponsorWhereUniqueInput>;
}
export { SponsorUpdateManyWithoutEventsInput };
